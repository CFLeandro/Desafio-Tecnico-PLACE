...
export class ListarCidades {
    ...
    public excluir(cidade: Cidade) {
        ...
        this.service.excluir(cidade).subscribe({
            ...
            // Sem esse tratamento, se o backend estiver fora do ar o usuário
            // ficaria clicando em excluir sem entender por que nada acontece
            error: () => {
                this.messageService.add({ severity: 'error', summary: 'Erro', detail: `Não foi possível excluir a cidade '${cidade.nome}'.` });
            }
        });
    }
